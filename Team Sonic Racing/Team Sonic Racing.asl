...
state("GameApp_PcDx11_x64Final", "v1.1")
{
	...
	byte gamemode: 0x1135E6C;							// Stays at 0 during Team Adventure mode. Becomes 1 during All Tracks categories
	...
	byte track: 0x11325A0, 0x11; 						// Internal ID of the selected track for Team Adventure mode
}
...
state("GameApp_PcDx11_x64Final", "v1.0")
{
	...
	byte gamemode: 0x112EBBC;							// Stays at 0 during Team Adventure mode. Becomes 1 during All Tracks categories
	...
	byte stars1: 0x1126B30, 0x4A14;						// Nr. of stars in Team Adventure Chapter 1
	byte stars2: 0x1126B30, 0x4A18;						// Nr. of stars in Team Adventure Chapter 2
	byte stars3: 0x1126B30, 0x4A1C;						// Nr. of stars in Team Adventure Chapter 3
	byte stars4: 0x1126B30, 0x4A20;						// Nr. of stars in Team Adventure Chapter 4
	byte stars5: 0x1126B30, 0x4A24;						// Nr. of stars in Team Adventure Chapter 5
	byte stars6: 0x1126B30, 0x4A28;						// Nr. of stars in Team Adventure Chapter 6
	byte stars7: 0x1126B30, 0x4A2C;						// Nr. of stars in Team Adventure Chapter 7
	...
	byte track: 0x112B210, 0x11; 						// Internal ID of the selected track for Team Adventure mode
}
...
update
{
	...
	// This code checks whether you are in the final challenge in adventure mode
	// and is needed to make LiveSplit split correctly at the credits screen
	if (current.gamemode == 0 && current.track == 104) {
		if (vars.finalsplit == 0) {
			if (current.credits == 4 || current.skippedcredits == 8) {
				vars.finalsplit = 1;
			}
		}
	}
...
split
{
	...
	if (current.gamemode == 0) {
		//  This is requirement is specific to the final event. It's untested outside he any% category
		if (current.track == 104) {
		...
		{
			// "current.requiredlaps != 0" is needed to avoid the game splitting at the start screen,
			// where it temporarily resets the number of stars gained in Team Adventure mode.
			// This will guarantee the game will only split when it's needed
			if (current.requiredlaps != 0) {
